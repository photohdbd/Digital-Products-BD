...
const Footer: React.FC = () => {
    return (
        <footer className="bg-base-200 text-secondary mt-16">
            <div className="container mx-auto px-6 py-10">
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
                    <div>
                        <h2 className="text-2xl font-bold text-white">Digi<span className="text-primary">BD</span></h2>
                        <p className="mt-2 text-sm text-secondary">Your trusted source for digital goods.</p>
                    ...
                    <div>
                        ...
                        <ul className="mt-4 space-y-2">
                            ...
                            <li><Link to="/contact" className="text-secondary hover:text-white">Contact Us</Link></li>
                    ...
                    <div>
                        ...
                        <ul className="mt-4 space-y-2">
                            ...
                            <li><Link to="/terms" className="text-secondary hover:text-white">Terms & Conditions</Link></li>
                            <li><Link to="/refund-policy" className="text-secondary hover:text-white">Refund Policy</Link></li>
                        </ul>
                    </div>
                    <div>
                        <h3 className="text-white font-semibold">Join Our Newsletter</h3>
                        <p className="text-secondary mt-2 text-sm">Join our list and be the first to receive exclusive offers, free gifts & resources.</p>
                        <form className="mt-4 flex">
                            <input type="email" placeholder="Enter your email" className="w-full bg-base-300 border border-base-300 rounded-l-md py-2 px-3 text-white focus:outline-none focus:ring-2 focus:ring-primary" />
                            <button type="submit" className="bg-primary hover:bg-opacity-80 text-white font-bold py-2 px-4 rounded-r-md">Join</button>
                        </form>
                    </div>
                </div>
                <div className="mt-10 border-t border-base-300 pt-6 text-center text-sm text-secondary">
                    <p>&copy; {new Date().getFullYear()} DigiBD. All rights reserved.</p>
                    <Link to="/admin" className="text-xs text-secondary/70 hover:text-secondary mt-2 inline-block opacity-50 hover:opacity-100">Admin</Link>
                </div>
            </div>
        </footer>
    );
