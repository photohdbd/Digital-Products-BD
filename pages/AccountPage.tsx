...
const AccountPage: React.FC = () => {
  ...
  const getStatusPill = (status: OrderStatus) => {
    ...
    switch (status) {
      case OrderStatus.Pending: return `${baseClasses} bg-yellow-500/20 text-yellow-300`;
      case OrderStatus.Processing: return `${baseClasses} bg-blue-500/20 text-blue-300`;
      case OrderStatus.OnTheWay: return `${baseClasses} bg-indigo-500/20 text-indigo-300`;
      case OrderStatus.Delivered: return `${baseClasses} bg-green-500/20 text-green-300`;
      case OrderStatus.Cancelled: return `${baseClasses} bg-red-500/20 text-red-300`;
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      ...
      <div className="bg-base-200 p-8 rounded-xl shadow-subtle border border-base-300 mb-8">
      ...
      <div className="bg-base-200 p-8 rounded-xl shadow-subtle border border-base-300">
        <h2 className="text-2xl font-bold text-white mb-4">Order History</h2>
        <p className="text-gray-400 mb-6">Track the status of your recent orders here.</p>
        {userOrders.length > 0 ? (
              ...
              <tbody>
                {userOrders.map(order => (
                  <tr key={order.id} className="border-b border-base-300 last:border-0 hover:bg-base-300/50">
                    <td className="p-4 font-mono text-sm text-gray-400">#{order.id.split('-')[1]}</td>
