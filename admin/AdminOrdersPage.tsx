  ...
  return (
    <div className="space-y-8">
      ...
      <div className="space-y-6">
        {orders.map(order => (
          <div key={order.id} className={`bg-base-200 p-6 rounded-xl shadow-lg border-l-4 ${getStatusColor(order.status)}`}>
            ...
            <div className="flex flex-col md:flex-row justify-between md:items-center">
              <div>
                <p className="text-gray-400">Payment: <span className="font-semibold text-white">{order.paymentMethod}</span></p>
                {order.transactionId && <p className="text-gray-400">TxID: <span className="font-semibold text-white">{order.transactionId}</span></p>}
              </div>
              <div className="flex items-center space-x-2 mt-4 md:mt-0">
                ...
                <select
                  ...
                  className="bg-base-300 border border-base-100 rounded-md py-1 px-2 text-white"
                >
        ...
        {orders.length === 0 && <p className="text-center p-8 text-gray-400 bg-base-200 rounded-xl">No orders have been placed yet.</p>}
      </div>
    </div>
  );
};
